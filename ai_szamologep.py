...
class Muveletek:
    def osszeadas(self, szam1, szam2):
        return szam1 + szam2

    def kivonas(self, szam1, szam2):
        return szam1 - szam2

    def szorzas(self, szam1, szam2):
        return szam1 * szam2

    def osztas(self, szam1, szam2):
        if szam2 != 0:
            return szam1 / szam2
        else:
            return "Hibás bemenet: osztás nullával"
    ...
    def Futas(self):
        while True:
            ...
            if valasztas in ("1", "2", "3", "4"):
                ...
                if valasztas == "1":
                    eredmeny = self.calculator.osszeadas(szam1, szam2)
                    muvelet = "+"
                elif valasztas == "2":
                    eredmeny = self.calculator.kivonas(szam1, szam2)
                    muvelet = "-"
                elif valasztas == "3":
                    eredmeny = self.calculator.szorzas(szam1, szam2)
                    muvelet = "*"
                elif valasztas == "4":
                    eredmeny = self.calculator.osztas(szam1, szam2)
                    muvelet = "/"
